import PySimpleGUI as sg
import fileinput
import pandas as pd
import xlsxwriter
import shutil,os
import calculationConvert as cc
import calcCheatSheet as cs


blue_dots = b''
txt = ['$$', 'if(', 'in(', 'match(','if (','in (',' date(','format(','component(','isnull','not isnull','ST_','days(',
       'adddays(','daysbetween(','quarter(','strlen(','seconddate(','int(','double(','timestamp(','+','*']
convtxt = ['$$', 'if(','if (','format(','component(','ST_','adddays(','daysbetween(','quarter(','strlen(','seconddate(',' int(',' double(',' timestamp(','+','*','isnull','ISNULL','isNull']
multilineText = 'Please input the correct SQL format for the calculated column\non the left in this block else leave it as it is'
DownloadTip = '''Please use this to download all the calculations to an excel file\nand convert them to the desired SQL format.\nOnce done those can be copied and pasted in the\nconverted window and submitted '''
highlightTip = '''Please use this button to highlight the\nissue causing parts in the calculations and filters\nwhich needs to be changed.\nthis is a static list and there may\nbe more issue keywords'''
remHighlightTip = '''Please use this button to remove the highlighted keywords'''
SubmitTip = '''Please use this button to submit\nthe changes and replace in the sql file which\n will be downloaded once the process completes'''

def dictMerge(keyDict,ValDict):
    outDict ={}
    for outkey,outval in keyDict.items():
        outDict[outval] = ValDict[outkey]
    return outDict


def textChange(file,toBeRep,repWith):
    with open(file, "r+") as fil:
        contents = fil.read()
        contents = contents.replace(toBeRep, repWith)
        contents = contents.replace('TO_TO_', 'TO_')
        fil.seek(0)

        fil.write(contents)

        fil.truncate()

def change_dict_key(d, old_key, new_key, default_value=None):
    d[new_key] = d.pop(old_key, default_value)


def changeColor(inpStr,texter):
    # for pattern in texter:
    patter_len = len(texter)
    res = [i for i in range(len(inpStr)) if inpStr.lower().startswith(texter, i)]
    if None in res:
        pass
    else:
        indexes = []
        for num in res:
            startIndex = f'1.{num}'
            endIndex = f'1.{num+patter_len}'
            indexes.append((startIndex,endIndex))
    return indexes

font1 = ('Courier New', 10)
font2 = ('Courier New', 10, 'bold')

def custom_meter_example():
    # layout the form
    layout = [ [sg.ProgressBar(1, orientation='h', size=(20, 20), key='progress')]]
    # create the form`
    windowcustombar = sg.Window('', layout,no_titlebar=True,grab_anywhere=True,modal=True)
    progress_bar = windowcustombar['progress']
    # loop that would normally do something useful
    for i in range(2000):
        # check to see if the cancel button was clicked and exit loop if clicked
        event, values = windowcustombar.read(timeout=0)
        if event == 'Cancel' or event == None:
            break
        # update bar with loop value +1 so that bar eventually reaches the maximum
        progress_bar.update_bar(i+1, 2000)
    # done with loop... need to destroy the window as it's still open
    windowcustombar.close()


def show_window(CalcCol,FilterCol,filetype,InpCol=None):
    CalcConvertDictionary = {}
    FilterConvertDictionary ={}
    calcConvertedDict ={}
    filterConvertedDict ={}
    filDict = {}
    calcDict = {}
    for filnum in range(0, len(FilterCol)):
        filDict[f'FILTER{filnum}'] = FilterCol[filnum]
    # print(filDict)
    for calcnum in range(0, len(CalcCol)):
        calcDict[f'CALC{calcnum}'] = CalcCol[calcnum]

    calcConvertedDict = cc.calculationConvert(calcDict)
    filterConvertedDict = cc.calculationConvert(filDict)
    sg.theme_add_new(
        'NewTheme18878',
        {
            'BACKGROUND': '#dfedf2',
            'TEXT': '#000000',
            'INPUT': '#183440',
            'SCROLL': '#dfedf2',
            'TEXT_INPUT': '#FFFFFF',
            'BUTTON': ('#dfedf2', '#183440'),
            'PROGRESS': ('#6cb6ff', '#000000'),
            'BORDER': 1,
            'SLIDER_DEPTH': 0,
            'PROGRESS_DEPTH': 0,
        }

    )

    sg.theme('NewTheme18878')
    calcLen = len(CalcCol)*100
    filterLen = len(FilterCol) * 100
    InpLen = len(InpCol) * 100
    windoSize = calcLen + filterLen
    windowheight = calcLen + filterLen+InpLen
    if calcLen > 300:
        calcLen = 300
    else:
        if calcLen == 0:
            calcLen =100
        else:
            pass
    if filterLen > 300:
        filterLen = 300
    else:
        if filterLen == 0:
            filterLen = 100
        else:
            pass
    if InpLen > 300:
        InpLen = 300
    else:
        if InpLen == 0:
            InpLen = 100
        else:
            pass
    if windoSize > 1000:
        windoSize = 1270
        windowheight = 1000
    else:
        windoSize =1270
        windowheight = calcLen + filterLen+InpLen+200
    # print(windowheight)
    # if InpCol is not None:

    InpHeadLayout = [sg.Text('\tHANA TABLE FUNCTION INPUT PARAMETERS\t\t\t\t\t\tVALUES TO BE PUSHED FOR THE INPUT PARAMETERS',font=font2)]

    Inputs = [[sg.Column([[sg.Multiline(f'{InpCol[i]}', size=(60, 5), expand_x=True, no_scrollbar=False,enable_events=True, key=f'-MLINEINP{i}-', border_width=3, disabled=True,font=font2),
                    sg.VSeparator(),sg.Multiline(multilineText,size=(80, 5), expand_x=True, enable_events=True, no_scrollbar=True,key=f'-CRINEINP{i}-',font=font2)] for i in range(0, len(InpCol))],
                    expand_x=True, expand_y=True, scrollable=True, vertical_scroll_only=True)]]
    Inputs.insert(0,InpHeadLayout)

    calcHeadLayout = [sg.Text('\tHANA CALCULATIONS DONE IN THE CALCULATED COLUMNS\t\t\t\tHANA SQL COUNTERPART TO BE USED FOR SQL QUERY',font=font2)]
    calc_columns = [[sg.Column([[sg.Multiline(f'{calcDict[calkey]}', size=(60, 5), expand_x=True, no_scrollbar=False,
                                              enable_events=True, background_color='grey', key=f'-MLINEC{calcindex}-',
                                              border_width=3, disabled=True,
                                              font=font2),
                                 sg.VSeparator(),
                                 sg.Multiline(f'{calvalue}', size=(80, 5), expand_x=True, enable_events=True,
                                              no_scrollbar=True, font=font2, key=f'-CRINEC{calcindex}-')] for
                                calcindex, (calkey, calvalue) in enumerate(calcConvertedDict.items())],
                               expand_x=True, expand_y=True, scrollable=True, vertical_scroll_only=True)]]
    calc_columns.insert(0,calcHeadLayout)

    #
    filterHeadLayout = [sg.Text('\tHANA FILTER COLUMNS IN GRAPHICAL VIEW\t\t\t\t\t\tHANA CONVERTED FILTER VALUES FOR SQL', font=font2)]
    filter_columns = [[sg.Column([[sg.Multiline(f'{filDict[filkey]}', size=(60, 5), expand_x=True, no_scrollbar=False,
                                                enable_events=True, background_color='grey', key=f'-MLINEF{filindex}-',
                                                border_width=3, disabled=True,
                                                font=font2),
                                   sg.VSeparator(),
                                   sg.Multiline(f'{filvalue}', size=(80, 5), expand_x=True, enable_events=True,
                                                no_scrollbar=True, font=font2, key=f'-CRINEF{filindex}-')] for
                                  filindex, (filkey, filvalue) in enumerate(filterConvertedDict.items())],
                                 expand_x=True, expand_y=True, scrollable=True, vertical_scroll_only=True)]]
    filter_columns.insert(0,filterHeadLayout)

    col_1 = [sg.Frame('CALCULATED COLUMNS', layout= calc_columns,element_justification='left',expand_y=True, expand_x=True,size=(300,calcLen))]
    col_2 = [sg.Frame('FILTER COLUMNS', layout=filter_columns,element_justification='left',expand_x=True,expand_y=True,size=(300,filterLen))]
    col_3 = [sg.Frame('INPUTS FOR FUNCTIONS', layout=Inputs, element_justification='left', expand_x=True, expand_y=True,size=(300, InpLen))]
    final = [[sg.Button('Highlight issue Keywords',key='-HIGHLIGHT-',tooltip=highlightTip ,enable_events=True),sg.Button('Remove Highlight',key='-REMOVE-',tooltip=remHighlightTip ,enable_events=True),
              sg.Button('Download To Excel',key='-DNWLDEXCEL-',tooltip=DownloadTip, enable_events=True),sg.Button('Open Conversion Cheatsheet',key='-CONVCS-',tooltip=DownloadTip, enable_events=True),sg.Button('Submit',key='-SUBMIT-',tooltip=SubmitTip, enable_events=True)],col_3,col_1,col_2]
    lay = [[sg.Frame('CONVERT CALCULATIONS AND FILTERS TO SQL FORMAT', layout= final,expand_x=True,expand_y=True)]]
    # lay = [sg.Col(final,scrollable=True)]
    layout = lay

    window = sg.Window('Convert Graphical Calculations to SQl', layout,return_keyboard_events=True,modal=True,size=(windoSize,windowheight))  #size=(1000,800)
    window.set_icon(r'logo.ico')
    outval ='NOGO'
    while True:  # Event Loop
        event, values = window.read() #type:[],{}
        # print('============ Event = ', event, ' ==============')
        # print('-------- Values Dictionary (key=value) --------')
        # for key in values:
        #     print(key, ' = ', values[key])
        # print(event, values)
        if event == sg.WIN_CLOSED or event == 'Exit':
            # window.close()
            break
        elif event == '-REMOVE-':
            for key in values:
                if '-MLINE' in key:
                    multiline = window[key]
                    widget = multiline.Widget
                    widget.tag_config('HIGHLIGHT', foreground='white', background='blue', font=font2)
                    for pattern in txt:
                        changeIndex = changeColor(values[key],pattern)
                        for index1, index2 in changeIndex:
                            widget.tag_remove('HIGHLIGHT', index1, index2)
                if '-CRINE' in key:
                    multiline = window[key]
                    widget = multiline.Widget
                    widget.tag_config('HIGHLIGHT', foreground='white', background='blue', font=font2)
                    for pattern in convtxt:
                        changeIndex = changeColor(values[key],pattern)
                        for index1, index2 in changeIndex:
                            widget.tag_remove('HIGHLIGHT', index1, index2)

        elif event == '-HIGHLIGHT-':
            for key in values:
                if '-MLINE' in key:
                    multiline = window[key]
                    widget = multiline.Widget
                    widget.tag_config('HIGHLIGHT', foreground='white', background='blue', font=font2)
                    for pattern in txt:
                        changeIndex = changeColor(values[key],pattern)
                        for index1, index2 in changeIndex:
                            widget.tag_add('HIGHLIGHT', index1, index2)
                if '-CRINE' in key:
                    multiline = window[key]
                    widget = multiline.Widget
                    widget.tag_config('HIGHLIGHT', foreground='white', background='blue', font=font2)
                    for pattern in convtxt:
                        changeIndex = changeColor(values[key],pattern)
                        for index1, index2 in changeIndex:
                            widget.tag_add('HIGHLIGHT', index1, index2)

        elif event == '-DNWLDEXCEL-':
            workbook = xlsxwriter.Workbook('CalcToBeConverted.xlsx')
            worksheet = workbook.add_worksheet()
            worksheet.write('A1','CALCULATED COLUMNS')
            worksheet.write('B1', 'CALC CONVERSION')
            worksheet.write('C1', 'FILTER COLUMNS')
            worksheet.write('D1', 'FILTER CONVERSION')
            worksheet.write('E1', 'INPUT COLUMNS')
            worksheet.write('F1', 'INPUT VALUES')

            calculations = dictMerge(calcDict,calcConvertedDict)
            filterColumns = dictMerge(filDict,filterConvertedDict)
            InpColumns =InpCol
            # print(calculations)
            # print(filterColumns)

            row =1
            columnCalc = 0
            columnConv = 1
            for item,val in calculations.items():
                item = item.replace('\r', '')
                val = val.replace('\r', '')
                worksheet.write(row, columnCalc, item)
                worksheet.write(row, columnConv, val)
                row += 1

            row = 1
            columnFilt = 2
            columnFiltConv = 3
            for item,val in filterColumns.items():
                item = item.replace('\r', '')
                val = val.replace('\r', '')
                worksheet.write(row, columnFilt, item)
                worksheet.write(row, columnFiltConv, val)
                row += 1

            row = 1
            column = 4
            for item in InpColumns:
                item = item.replace('\r', '')
                worksheet.write(row, column, item)
                row += 1

            workbook.close()
            script_dir = os.path.dirname(os.path.abspath(__file__))
            source_file_path = os.path.join(script_dir, 'CalcToBeConverted.xlsx')
            filename = sg.PopupGetFile('Save Settings', file_types=(("All excel Files", "*.xlsx"),), save_as=True,
                                       no_window=True)
            try:
                shutil.copy(source_file_path, filename)
            except Exception as e:
                sg.popup(e)
            else:
                sg.popup(
                    f"Calculations to be converted \nhave been downloaded to: {filename}",
                    title='Download Complete')


        elif event == '-CONVCS-':
            try:
                cs.show_window()
            except Exception as e:
                print(e)


        elif event == '-SUBMIT-':
            toBeChange =[]
            toChangeWith =[]
            valDict =  {}
            for key in values:
                if '-MLINE' in key:
                    toBeChange.append(key)
                elif '-CRINE' in key:
                    toChangeWith.append(key)
            for i in range(0,len(toBeChange)):
                valDict[values[toBeChange[i]]] = values[toChangeWith[i]]

            for toChange,changeWith in valDict.items():
                if filetype == 'SQL':
                    if changeWith == multilineText:
                        pass
                    elif changeWith == '':
                        textChange('SQLOUT.sql', f'WHERE {toChange}', changeWith)
                    else:
                        textChange('SQLOUT.sql', toChange, changeWith)
                elif filetype == 'DOBEGIN':
                    if changeWith == multilineText:
                        pass
                    elif changeWith == '':
                        textChange('OUTPUT.sql', f'WHERE {toChange}', changeWith)
                    else:
                        textChange('OUTPUT.sql',toChange,changeWith)
                elif filetype == 'SNWSQL':
                    if changeWith == multilineText:
                        pass
                    elif changeWith == '':
                        textChange('SNOWOUTPUT.sql', f'WHERE {toChange}', changeWith)
                    else:
                        textChange('SNOWOUTPUT.sql',toChange,changeWith)
                elif filetype == 'SNOW_SQL':
                    if changeWith == multilineText:
                        pass
                    elif changeWith == '':
                        textChange('SNOWSQLOUT.sql', f'WHERE {toChange}', changeWith)
                    else:
                        textChange('SNOWSQLOUT.sql',toChange,changeWith)
            outval = 'GO'
            break

            window.close()
    window.close()
    return outval



